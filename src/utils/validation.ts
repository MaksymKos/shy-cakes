...
export function validateCreateUser(data: any): CreateUser {
...
export function validateCreateOrder(data: any): CreateCakeOrder {
...
export function validateCreateReview(data: any): CreateReview {
  if (!isValidObjectId(data.userId)) {
    throw new ValidationError('Невірний ID користувача');
  }
...
export function validatePaginationOptions(options: any): PaginationOptions {
  return {
    page: Math.max(1, parseInt(options.page) || 1),
    limit: Math.min(100, Math.max(1, parseInt(options.limit) || 10)),
